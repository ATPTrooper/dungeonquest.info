...
<head>
	...
	<script type="text/javascript">
		...
		function remove_movement_encounters() {
			...
			for (var i = encounter_queue.length-1; i >= 0; i--) {
				if (encounter_queue[i].type == 'opendoor' || 
					...
					encounter_queue[i].type == 'speardoor' || 
		...
		$(document).ready(function() {
			...
			$('#trap_modal').on('hidden.bs.modal', function () {
				...
				if (encounter_queue[0] != undefined) {
					...
					// movement encounters
					// if fail movement test, or cancel movement, remove all movement encounters
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								if (encounter_queue.length > 1) {
									write_to_debugger("Movement[0] Finished: "+ encounter_queue[0].type + ", Second Encounter Started: " + encounter_queue[1].type);
									encounter_queue.splice(0, 1);
									force_trap_modal();
									$('#trap_modal').show();
								} else {
									write_to_debugger("Movement[0] Successful: "+ encounter_queue[0].type + ", No Second Encounter");
		...
		function draw_chamber(p_direction) {
			...
			if (!is_exit_chamber(hero1) && !is_treasure_chamber(hero1)) {
				...
				if (hero_map[eval(hero1.x*100 + hero1.y)] == undefined) { 
					...
					write_to_debugger('Draw Tile: ' + randomChamber.id + ": " + new_top + " " + new_right + " " + new_bottom + " " + new_left + " " + randomChamber.type);
		...
		function search_crypt(){
			...
			if (debug || parseInt(curr_hero_chamber.crypt) > 0) {
				if (debug || parseInt(curr_hero_chamber.crypt_searched) < 1) {
					...
					//write_to_debugger("Search Crypt: " + randomDraw.id + ": " + randomDraw.type);
		...
		function search_corpse(){
			...
			if (debug || parseInt(curr_hero_chamber.corpse) > 0) {
				if (debug || parseInt(curr_hero_chamber.corpse_searched) < 1) {
					...
					//write_to_debugger("Search Corpse: " + randomDraw.id + ": " + randomDraw.type);
		...
		function search_dragon_chamber(){
			...
			if ((debug || hero_map[hero1.x*100 + hero1.y].type == 'treasure') && 
				...
				if (String(randomDraw.type) == 'sleeping') {
					...
					write_to_console('The Dragon is Sleeping');
					
					if (!treasure_deck.empty()) {
						play_audio("gold");
						randomDraw = treasure_deck.draw_unique_card();
						add_to_loot_bag(randomDraw.id, randomDraw.name, randomDraw.value, '', '');
						write_to_console('You loot ' + randomDraw.name);
					}
					...
					write_to_console('You woke the Dragon');
					write_to_console('You drop your Loot Bag');
					drop_loot_bag();
		...
		function encounter_chamber() {
			...
			write_to_debugger("Encounter Chamber: " + randomDraw.id + ": " + randomDraw.type);
		...
		function roll_trap_dice() {
			...
			write_to_debugger("Roll Trap Dice: " + encounter_queue[0].type);
			...
			switch(encounter_queue[0].type) {
				...
				case "collapse":
					...
					if (parseInt(total) >= 11) {
						play_audio("hero_dying");
						write_to_console('['+total+'] You were killed from the Collapsing Chamber!');
						$('.trap_text').html("You were killed from the Collapsing Chamber!");
						$('.dice_total').css("background-color", "#D1001C");
						$('.dice_total').css("color", "white");
