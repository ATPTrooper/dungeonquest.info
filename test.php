...
<head>
	...
	<script type="text/javascript" src="/dungeonquest_second_edition.js"></script>
	...
	<script type="text/javascript">
		...
		var image_directory = "/images/second_edition_friendlybombs/";
		var chamber_image_directory = "/images/second_edition/";
		var sound_directory = "/sound/";
		var audio;
		
		function play_audio(p_type){
			for (var i = 0; i < SoundEffectJSON.length; i++) {
				if (p_type == SoundEffectJSON[i].name) {
					audiosWeWantToUnlock[i].play();
				}
			}
		}
		...
		function preload_audio() {
			for (var i = 0; i <  audiosWeWantToUnlock.length; i++) {
				try {
					audiosWeWantToUnlock[i].play();
					audiosWeWantToUnlock[i].pause();
					audiosWeWantToUnlock[i].currentTime = 0;
				}
				catch(err) {
				}
			}
		...
		$(document).ready(function() {
			...
			for (var i = 0; i < SoundEffectJSON.length; i++) {
				audiosWeWantToUnlock.push(new Audio('/sound/'+SoundEffectJSON[i].file));
			}
			...
			$('#encounter_modal').on('show.bs.modal', function () {
				...
				write_to_debugger("Show Encounter Modal: " + encounter_queue[0].type);
				...
				if (current_encounter != undefined) {
					...
					$('.encounter_ambushed_button').hide();
					$('.encounter_shoot_first_arrow_button').hide();
					$('.encounter_shoot_second_arrow_button').hide();
					$('.encounter_combat_button').hide();
					$('.encounter_wait_button').hide();
					$('.encounter_escape_button').hide();
					$('.encounter_fight_button').hide();
					
					if (current_encounter.show_combat_buttons == '1') {
						setup_combat_buttons();
					}
			...
			$('#encounter_modal').on('hidden.bs.modal', function () {
				...
				$('.mini_dice_placeholder').attr("onclick", "").unbind("click");
				$(".mini_dice_placeholder").on('click', function(){ roll_dice(); });
				...
				// debug
				write_to_debugger("Close Encounter Modal: " + encounter_queue[0].type);
				write_to_debugger("Encounter Queue Length: " + encounter_queue.length);
				write_to_debugger("Encounter Queue Start of List");

				for (var i = 0; i < encounter_queue.length; i++) {
					write_to_debugger("Encounter ["+i+"]: " + encounter_queue[i].type + ", R = "+encounter_queue[i].resolved+", S = "+encounter_queue[i].success);
				}
				write_to_debugger("Encounter Queue End of List");
				...
				if (encounter_queue[0] != undefined) {
					...
					if (encounter_queue[0].type == "opendoor" || 
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								// Cancel all movement if fail
								remove_movement_encounters();
								update_timer();
								return;
							}
						} else {
							write_to_debugger("Movement: "+encounter_queue[0].type+": R = 0");
							
							// Cancel all movement except when encountering doortrap
					...
					if (
						// TRAP
						encounter_queue[0].type == "trapdoor" ||
						...
						// CHAMBER ENCOUNTER
						encounter_queue[0].type == "vampirebats" ||
						...
						encounter_queue[0].type == "ambushgoblin" ||
						encounter_queue[0].type == "ambushskeleton" ||
						encounter_queue[0].type == "ambushtroll" ||
						...
						// CRYPT
						encounter_queue[0].type == "cryptalreadysearched" ||
						...
						// CORPSE
						encounter_queue[0].type == "corpsealreadysearched" ||
						encounter_queue[0].type == "emptycorpse" ||
						encounter_queue[0].type == "findcorpseitem" ||
						encounter_queue[0].type == "scorpion" ||
						
						// DRAGON BREATH
						encounter_queue[0].type == "dragonbreath" ||
						
						// POTION
						encounter_queue[0].type == "potion") {
						
						
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Encounter [0]: "+ encounter_queue[0].type+": R = 1, S = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Encounter [0]: "+ encounter_queue[0].type+": R = 1, S = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Encounter [0]: "+ encounter_queue[0].type+": R = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "torchout") {
						if (encounter_queue[0].resolved == '1') {
							...
							write_to_debugger("Encounter [0]: "+ encounter_queue[0].type+": R = 0, S = 0");
							return;
						}
					}
		...
		function load_characters(){
			for (var i = 0; i < CharactersJSON.length; i++){
				...
				$character_tbody.append('<tr><td rowspan="5" class="character_sheet_image" style="padding: 0px; margin: 0px;">'+
		...
		function draw_hero_stats(p_hero_num, p_hero) {
			...
			$('.'+p_hero_num+'_arrows').html(p_hero.arrows);
			...
			for (var i = -1; i < 3; i++) {
				for (var j = -1; j < 2; j++) {
					if (hero1 != undefined) {
						...
						var background_rotation = empty_map_orientation[eval((hero1.x+i+x_adjustment)*100 + (hero1.y+j+y_adjustment))];
		...
		function reset_game_board(){
			...
			// load corner tiles
			hero_map[100 + 1] = new Chamber(0, 1, 1, 2, 2, 'starting', get_rotation_angle(4), 'corner.jpg', 'South West Tower');
			hero_map[boardgame_width*100 + 1] = new Chamber(0, 1, 2, 2, 1, 'starting', get_rotation_angle(3), 'corner.jpg', 'North West Tower');
			hero_map[100 + boardgame_height] = new Chamber(0, 2, 1, 1, 2, 'starting', get_rotation_angle(1), 'corner.jpg', 'South East Tower');
			hero_map[boardgame_width*100 + boardgame_height] = new Chamber(0, 2, 2, 1, 1, 'starting', get_rotation_angle(2), 'corner.jpg', 'North East Tower');
			
			// load treasure tiles
			hero_map[705] = new Chamber(0, 1, 1, 1, 1, 'treasure', 0, 'treasure_bottom.jpg', 'Treasure Chamber');
			hero_map[706] = new Chamber(0, 1, 1, 1, 1, 'treasure', 0, 'treasure_top.jpg', 'Treasure Chamber');
			...
			// draw treasure tiles
			$('#dungeon_board_cell_7_5').css('background-image','url("/images/second_edition_friendlybombs/treasure_bottom.jpg")');
			$('#dungeon_board_cell_7_6').css('background-image','url("/images/second_edition_friendlybombs/treasure_top.jpg")');
		...
		function remove_heros() {
			...
			for (var i = -1; i < 3; i++) {
				for (var j = -1; j < 2; j++) {
					$('#mini_board_cell_'+(i)+'_'+(j)).empty();
				}
			}
			for (var i = -2; i < 3; i++) {
				for (var j = -2; j < 3; j++) {
					$('#medium_board_cell_'+(i)+'_'+(j)).empty();
				}
			}
		}
				...
				if (debug || parseInt(curr_hero_chamber.searched) < 2) {
					...
					switch (randomDraw.type) {
						...
						case "finditem":
							...
							//add_encounter("finditem", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
						case "centipede":
							play_audio("centipede");
							write_to_console('Search: Giant Centipede!');
							add_encounter("centipede", "0", randomDraw.image_url)
							force_encounter_modal();
							break;
						case "trap":
						...
						case "passagedown":
							...
							//add_encounter("passagedown", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
					}
				...
				if (debug || parseInt(curr_hero_chamber.crypt_searched) < 1) {
					...
					switch (randomDraw.type) {
						case "emptycrypt":
							...
							//add_encounter("emptycrypt", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
						case "finditem":
							play_audio("gold");
							...
							//add_encounter("findcryptitem", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
						case "crypttrap":
							write_to_console('Crypt: Trap!');
							add_encounter("crypttrap", "0", randomDraw.image_url)
							force_encounter_modal();
							break;
						case "skeleton":
							write_to_console('Crypt: Skeleton');
							add_encounter("skeleton", "0", randomDraw.image_url)
							force_encounter_modal();
							break;
					}
				...
				if (debug || parseInt(curr_hero_chamber.corpse_searched) < 1) {
					...
					switch (randomDraw.type) {
						case "emptycorpse":
							...
							//add_encounter("emptycorpse", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
						case "finditem":
							play_audio("loot");
							...
							add_to_loot_bag(randomDraw.id, randomDraw.name, randomDraw.value, '', '');
							//add_encounter("findcorpseitem", "0", randomDraw.image_url);
							//force_encounter_modal();
							break;
						case "scorpion":
							write_to_console('Corpse: Scorpion!');
							add_encounter("scorpion", "0", randomDraw.image_url)
							force_encounter_modal();
							break;
					}
		...
		function search_dragon_chamber(){
			...
			if ((debug || hero_map[hero1.x*100 + hero1.y].type == 'treasure') && 
				...
				if (String(randomDraw.type) == 'sleeping') {
					...
					empty_loot_bag();
					add_encounter("dragonbreath", "0", randomDraw.image_url);
					force_encounter_modal();
		...
		function encounter_chamber() {
			...
			var randomDraw = chamber_event_deck.draw_card();
			...
			switch (randomDraw.type) {
				...
				case "trapdoor":
					...
					add_encounter("trapdoor", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "crossfire":
					write_to_console('Encounter: Hidden Arrows!');
					add_encounter("crossfire", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "passagedown":
				...
				case "collapse":
					...
					add_encounter("collapse", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "torchout":
					write_to_console('Encounter: Torch Goes Out');
					hero1.torch = '0';
					add_encounter("torchout", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "secretdoor":
				...
				case "vampirebats":
					...
					add_encounter("vampirebats", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "wizardscurse":
					write_to_console('Encounter: Wizard\'s Curse');
					add_encounter("wizardscurse", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "finditem":
					play_audio("loot");
					if (randomDraw.name == "Potion" && have_item("Potion")) {
				...
				case "giantspider":
					...
					add_encounter("giantspider", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "goblin":
					play_audio("monster_wounded");
					write_to_console('Encounter: Goblin');
					add_encounter("goblin", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "orc":
					play_audio("monster_wounded");
					write_to_console('Encounter: Orc');
					add_encounter("orc", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "troll":
					play_audio("monster_wounded");
					write_to_console('Encounter: Troll');
					add_encounter("troll", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "undead":
					play_audio("monster_wounded");
					write_to_console('Encounter: undead');
					add_encounter("undead", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "blackknight":
					play_audio("monster_wounded");
					write_to_console('Encounter: Black Knight');
					add_encounter("blackknight", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "ambushgoblin":
					play_audio("monster_wounded");
					write_to_console('Encounter: Ambush Goblin');
					add_encounter("ambushgoblin", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "ambushskeleton":
					play_audio("monster_wounded");
					write_to_console('Encounter: Ambush Skeleton');
					add_encounter("ambushskeleton", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
				case "ambushtroll":
					play_audio("monster_wounded");
					write_to_console('Encounter: Ambush Troll');
					add_encounter("ambushtroll", "0", randomDraw.image_url)
					force_encounter_modal();
					break;
			}
		}
		...
		function draw_monster_stats(p_monster) {
			...
			if (encounter_queue[0].show_combat_buttons == '1') {
				$('.battle_monster_health').html("?");
			}
		...
		function trigger_trap() {
			...
			switch(randomDraw.type) {
				...
				case "snakes":
					...
					add_encounter("snakes", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "gas":
					play_audio("gas");
					write_to_console('Trap: Poisonous Gas');
					add_encounter("gas", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "collapse":
					play_audio("collapse");
				...
				case "explosion":
					...
					add_encounter("explosion", "0", randomDraw.image_url);
					force_encounter_modal();
					break;
				case "crossfire":
					play_audio("arrow");
		...
		function add_encounter(type, p_direction, p_image_url) {
			...
			switch (type) {
				...
				// MOVEMENT
				case "door":
					randomDoor = door_deck.draw_card();
				...
				case "goblin":
					...
					new_encounter.show_combat_buttons = '1';
					new_encounter.before_combat = '1';
					encounter_queue.push(new_encounter);
					
					randomDraw = monster_deck.draw_monster("goblin");
					write_to_debugger(randomDraw.name + " " + randomDraw.combat + " " + randomDraw.wait + " " + randomDraw.escape + " " + randomDraw.penalty_dice + " " + randomDraw.penalty);
					monster = new Monster(randomDraw);
					break;
				case "troll":
					play_audio("monster_wounded");
				...
				case "orc":
					play_audio("monster_wounded");
					new_encounter = new Encounter('orc', 'Orc', '#D1001C', 'white', 'You encounter an Orc!', p_direction);
					new_encounter.show_combat_buttons = '1';
					new_encounter.before_combat = '1';
					encounter_queue.push(new_encounter);
				...
				case "ambushgoblin":
					play_audio("monster_wounded");
					new_encounter = new Encounter('ambushgoblin', 'Goblin', '#D1001C', 'white', 'Ambush: Goblin! <br/> Damage: D12 - Luck', p_direction);
					new_encounter.show_combat_buttons = '1';
					new_encounter.before_combat = '1';
					new_encounter.ambushed = '1';
					new_encounter.can_wait = '0';
					new_encounter.can_shoot_first_arrow = '0';
					encounter_queue.push(new_encounter);
				...
				case "ambushtroll":
					play_audio("monster_wounded");
					new_encounter = new Encounter('ambushtroll', 'Troll', '#D1001C', 'white', 'Ambush: Troll! <br/> Damage: D12 - Luck', p_direction);
					new_encounter.show_combat_buttons = '1';
					new_encounter.before_combat = '1';
					new_encounter.ambushed = '1';
					new_encounter.can_wait = '0';
				...
				case "ambushskeleton":
					play_audio("monster_wounded");
					new_encounter = new Encounter('ambushskeleton', 'Skeleton', '#D1001C', 'white', 'Ambush: Skeleton! <br/> Damage: D12 - Luck', p_direction);
					new_encounter.show_combat_buttons = '1';
					new_encounter.before_combat = '1';
					new_encounter.ambushed = '1';
					new_encounter.can_wait = '0';
				...
				case "gas":
					play_audio("gas");
					new_encounter = new Encounter('gas', 'Poisonous Gas', '#D1001C', 'white', 'Poisonous Gas spills into the room! <br/> Damage: D6-3, Lose Time!', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "collapse":
					play_audio("collapse");
				...
				case "explosion":
					play_audio("explosion");
					new_encounter = new Encounter('explosion', 'Explosion', '#D1001C', 'white', 'You were hit by an explosion! <br/> Damage: D6, Miss a turn!', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "crossfire":
					play_audio("arrow");
			...
			switch(current_encounter.type) {
				...
				case "potion":
					...
					switch(total.toString()) {
						case "2":
							play_audio("potion");
							write_to_console("Potion: Heal 4 wounds");
							heal = 4;
							update_hero_health(heal);
							break;
						case "3":
							play_audio("potion");
							write_to_console("Potion: Heal 3 wounds");
							heal = 3;
							update_hero_health(heal);
							break;
						case "4":
							play_audio("potion");
							write_to_console("Potion: Heal 2 wounds");
							heal = 2;
							update_hero_health(heal);
							break;
						case "5":
							play_audio("potion");
							write_to_console("Potion: Heal 1 wound");
							heal = 1;
							update_hero_health(heal);
							break;
						case "6":
						...
						case "9":
							play_audio("potion");
							write_to_console("Potion: Nothing happens");
							$('.encounter_text').html("Nothing happens");
							break;
						case "10":
							play_audio("hero_wounded");
				...
				case "dragonbreath":
					...
					if (hero_dead()) {
						...
						if (!escape_closed()) {
							...
							if (hero_dead()) {
								play_audio("hero_crushed");
								write_to_console('Dragon: No Escape!');
								write_to_console('Dragon: You are killed by the Dragon!');
								$('.encounter_text').html("You are killed by the Dragon!");
								game_over = true;
							}
						}
				...
				case "ambushtroll":
					...
					if (p_button == 'fight') { 
						...
						current_encounter.can_shoot_first_arrow = '0';
						current_encounter.can_shoot_second_arrow = '0';
		...
		function escape_closed() {
			var current_chamber = hero_map[hero1.x*100 + hero1.y];
			var previous_chamber;
			
			write_to_debugger("previous direction: " + previous_direction);
			
			switch (previous_direction) {
				case '1':
					if (current_chamber.bottom == '5' || current_chamber.bottom == '2') return true;
					previous_chamber = hero_map[hero1.x*100 + hero1.y-1];
					if (previous_chamber.top == '5' || previous_chamber.top == '2') return true;
					break;
				case '2':
					if (current_chamber.left == '5' || current_chamber.left == '2') return true;
					previous_chamber = hero_map[(hero1.x-1)*100 + hero1.y];
					if (previous_chamber.right == '5' || previous_chamber.right == '2') return true;
					break;
				case '3':
					if (current_chamber.top == '5' || current_chamber.top == '2') return true;
					previous_chamber = hero_map[hero1.x*100 + hero1.y+1];
					if (previous_chamber.bottom == '5' || previous_chamber.bottom == '2') return true;
					break;
				case '4':
					if (current_chamber.right == '5' || current_chamber.right == '2') return true;
					previous_chamber = hero_map[(hero1.x+1)*100 + hero1.y];
					if (previous_chamber.left == '5' || previous_chamber.left == '2') return true;
					break;
			}
			return false
		}
			...
			for(var i = 0; i < hero_loot.length; i++){
				if (i == p_item_index){
					if (confirm("Do you want to drop: " + hero_loot[i].name)) {
						write_to_console("Drop Loot: " + hero_loot[i].name);
						hero_loot.splice(i, 1);
					}
				}
		...
		function have_item(p_item) {
			for (var i = 0; i < hero_loot.length; i++) {
				if (hero_loot[i].name == p_item) {
					return true;
				}
			}
			return false;
		...
		function event_not_resolved() {
			...
			if (encounter_queue.length > 0) {
				write_to_debugger("Event Not Resolved: " + encounter_queue[0].type);
				force_encounter_modal();
				return true;
			}
		...
		function battle_action(p_action) {
			
			var current_encounter = encounter_queue[0];
			
			current_encounter.show_combat_buttons = '0';
			current_encounter.show_fight = '1';
			current_encounter.can_fight = '1';
			...
			switch(p_action) {
				...
				case "ambushed":
					...
					if (hero_dead()) {
						write_to_console('['+total+'] You are killed by the '+monster.name+'!');
						$('.encounter_text').html("You are killed by the "+monster.name+"!");
						current_encounter.can_fight = '0';
						game_over = true;
					}
					
					current_encounter.ambushed = '0';
					current_encounter.before_combat = '1';
					current_encounter.can_shoot_first_arrow = '0';
					current_encounter.can_shoot_second_arrow = '0';
				...
				case "shoot_first_arrow":
				
					play_audio("arrow");
					play_audio("monster_wounded");
					hero1.arrows--;
					
					monster.health = monster.combat;
					update_hero_health(0);
					
					if (parseInt(monster.health) == 0) {
						play_audio("flee");
						if (monster.type == "blackknight") {
							write_to_console("Battle: Evaded" + monster.name);
							$('.encounter_text').html("You evaded " + monster.name);
						} else {
							$('.encounter_text').css("background-color", "#01E501");
							$('.encounter_text').css("color", "white");
							write_to_console("Battle: " + monster.name + " flees");
							$('.encounter_text').html(monster.name + " flees");
						}
						current_encounter.resolved = 1;
						current_encounter.success = 1;
						current_encounter.before_combat = '0';
						current_encounter.can_shoot_first_arrow = '0';
						current_encounter.can_shoot_second_arrow = '0';
						break;
					}
					...
					switch(total.toString()) {
						case "1":
						case "2":
							write_to_console("Battle: You shoot and missed " + monster.name);
							$('.encounter_text').html("You shoot and missed " + monster.name);
							break;
						case "3":
						case "4":
							dice2 = Math.floor(Math.random() * 6 + 1);
							...
							total = dice2;
							//$('.dice_total').html(total);
							wounds = Math.floor(total/2);
							update_monster_health(-wounds);
						
							play_audio("monster_wounded");
							write_to_console("Battle: You wounded " + monster.name + " " + wounds + " wounds");
							$('.encounter_text').html("You wounded " + monster.name + " " + wounds + " wounds");
							break;
						case "5":
						case "6":
							play_audio("monster_wounded");
							wounds = 0;
							write_to_console("Battle: Critical Shot!");
							$('.encounter_text').html("Critical Shot!");
							if (monster.type == 'blackknight') {
								wounds = 3;
								update_monster_health(-wounds);
								write_to_console("Battle: You wounded " + monster.name + " " + wounds + " wounds");
					...
					if (monster.health <= 0) {
						...
						$('.encounter_text').css("background-color", "#01E501");
						$('.encounter_text').css("color", "white");
						write_to_console("Battle: You killed " + monster.name);
						$('.encounter_text').html("You killed " + monster.name);
						current_encounter.resolved = 1;
						current_encounter.success = 1;
					}
					current_encounter.before_combat = '0';
					current_encounter.can_shoot_first_arrow = '0';
					current_encounter.can_shoot_second_arrow = '1';
					
					break;
				
				case "shoot_second_arrow":
					...
					write_to_debugger("shooting second arrow");
				...
				case "combat":
					play_audio("monster_wounded");
					write_to_console("Battle: Combat");
					monster.health = monster.combat;
					draw_monster_stats(monster);
					if (parseInt(monster.health) == 0) {
						play_audio("flee");
						...
						write_to_console("Battle: You begin fighting the " + monster.name);
						$('.encounter_text').html("You begin fighting the " + monster.name);
						current_encounter.resolved = 0;
						current_encounter.success = 0;
						draw_monster_stats(monster);
					}
					...
					break;
				case "wait":
					write_to_console("Battle: Wait and See");
					monster.health = monster.wait;
					draw_monster_stats(monster);
					if (parseInt(monster.health) == 0) {
						play_audio("flee");
						if (monster.type == "blackknight") {
						...
						write_to_console("Battle: You must fight the " + monster.name);
						$('.encounter_text').html("You must fight the " + monster.name);
						current_encounter.resolved = 0;
						current_encounter.success = 0;
						draw_monster_stats(monster);
					}
					...
					break;
				case "escape":
					monster.health = monster.escape;
					draw_monster_stats(monster);
					if (parseInt(monster.health) == 0 && !escape_closed()) {
						play_audio("flee");
						$('.encounter_text').css("background-color", "#01E501");
						$('.encounter_text').css("color", "white");
						write_to_console("Battle: Escape");
						$('.encounter_text').html("You Escaped");
						
						var wounds = 0;
						var penalty_dice = parseInt(monster.penalty_dice);
						var penalty = parseInt(monster.penalty);
						
						if (penalty_dice == 0) {
							
							wounds = -penalty;
							
						} else if (penalty_dice == 1) {
							dice1 = Math.floor(Math.random() * 6 + 1);
							mini_dice.dataset.side = dice1;
							mini_dice.classList.toggle("reRoll");
							...
							wounds = total - penalty;
							
						} else if (penalty_dice == 2) {
							dice1 = Math.floor(Math.random() * 6 + 1);
							mini_dice.dataset.side = dice1;
							mini_dice.classList.toggle("reRoll");
						...
						if (wounds > 0) {
							play_audio("hero_wounded");
							write_to_console("The " + monster.name +" attacks you!");
							write_to_console("You receive "+wounds+" wounds!");
							update_hero_health(wounds);
						}
						if (hero_dead()) {
							write_to_console('['+total+'] You are killed by the '+monster.name+'!');
						...
						play_audio("hero_wounded");
						write_to_console("Battle: Escape Failed");
						write_to_console("Battle: You must fight the " + monster.name);
						$('.encounter_text').html("You must fight the " + monster.name);
			...
			if (current_encounter.type == "ambushgoblin" || 
				current_encounter.type == "ambushskeleton" ||
				current_encounter.type == "ambushtroll") {
				current_encounter.can_shoot_first_arrow = '0';
				current_encounter.can_shoot_second_arrow = '0';
			}
		...
		function setup_combat_buttons() {
			...
			$('.encounter_shoot_first_arrow_button').prop("disabled", true);
			$('.encounter_shoot_second_arrow_button').prop("disabled", true);
			$('.encounter_combat_button').prop("disabled", false);
			$('.encounter_wait_button').prop("disabled", false);
			$('.encounter_escape_button').prop("disabled", false);
			$('.encounter_fight_button').prop("disabled", false);
			
			if (parseInt(hero1.arrows) > 0) {
				if (current_encounter.can_shoot_first_arrow == '1') { $('.encounter_shoot_first_arrow_button').prop("disabled", false); }
				if (current_encounter.can_shoot_second_arrow == '1') { $('.encounter_shoot_second_arrow_button').prop("disabled", false); }
			}
			
			if (current_encounter.can_wait == '0') { $('.encounter_wait_button').prop("disabled", true); }
			if (escape_closed()) { $('.encounter_escape_button').prop("disabled", true); }
			if (current_encounter.can_fight != '1') { $('.encounter_fight_button').attr("disabled", true); }
			
			// Combat Buttons
			if (current_encounter.ambushed == '1') {
				$('.encounter_ambushed_button').show();
			} else {
				
				if (current_encounter.before_combat == '1') {
					if (hero1.have_range == '1' && current_encounter.can_shoot_first_arrow == '1') {
						$('.encounter_shoot_first_arrow_button').show();
					}
					$('.encounter_combat_button').show();
					$('.encounter_wait_button').show();
					$('.encounter_escape_button').show();
				} else {
					if (hero1.have_range == '1') {
						$('.encounter_shoot_second_arrow_button').show();
					}
					$('.encounter_fight_button').show();
					
				}
				if (parseInt(monster.health) <= 0) {
					$('.encounter_shoot_second_arrow_button').prop("disabled", true);
					$('.encounter_fight_button').prop("disabled", true);
				}
			}
		...
		function print_board(){
			...
			var mapForm = document.createElement("form");
				...
				mapInput.type = "text";
				mapInput.name = " hero_map";
				mapInput.id = " hero_map";
				
				alert(hero_map[101].id);
				mapInput.value = hero_map;
...
.bg dungeon_board_cell_1_1 {
  @media print dungeon_board_cell_1_1 {
    visibility: visible;
...
<div id="main_screen" style="max-width: 1000px; margin: auto;">

<!-- Introduction Screen -->
<div class="introduction_screen" style="display: none; padding: 10px;">
		<div style="cursor: pointer;" onclick="introduction_click();">
			<img class="img img-responsive" style="margin: auto;" src="/images/second_edition/dungeonquest_title_name_800.jpg"/>
		</div>
		<div style="text-align: justify; max-width: 800px; margin: auto; padding: 0 20px 0 20px;">
		
			<p style="margin-bottom: 10px;">Hundreds of years have passed since the evil wizard fell, 
			but men still fear to enter his deadly fortress, which sits above the crag. 
			In the villages, stories are whispered of the treasures that fill its dungeons, and of the things that lurk within. 
			The old sit close to their fire and whisper about the noises that echo through the valley at night. 
			There are some who have entered and returned, but they are troubled and are reluctant to share their adventures. 
			Those who fail to get out by nightfall never return.</p>
			
			<p>As the light of dawn begin to rise, 
			four pairs of eyes look toward the looming keep. 
			They reflect on the villager's tales and tighten their grip on their weapons: 
			...
			El-Adoran the Ranger, with his longbow and shortsword; 
...
<div class="character_selection_screen" id="character_selection_screen" style="display: none; padding: 10px;">
	<div style="cursor: pointer;" onclick="title_click();">
		<img class="img img-responsive" style="margin: auto;" src="/images/second_edition/dungeonquest_title_name_800.jpg"/>
	</div>
	...
	<div class="div-desktop">
		...
		<div class="character_dashboard" id="character_dashboard" style="margin: 10px;">
			...
			<div class="hero_sheet">
				...
				<tbody>
				<tr>
					<td rowspan="5" style="width: 37%; padding: 0px;">
				...
				<tr>
					...
					<td style="width: 20%">🏹 <span class="hero1_arrows"></span></td>
				</tr>
				<tr>
					<td colspan="3">

						<!-- Dice Sheet -->
						<div class="dice_placeholder" onclick="roll_dice()" style="margin: 10px 0 10px 0;">
			...
			<!-- Control Buttons -->
			<div class="movement_dashboard" id="movement_dashboard" style="margin: auto;">
				...
				<input class="btn btn-primary rules_button" type="button" name="rules_button" id="rules_button" value="About" onclick="show_rules();" />
		...
		<div class="character_dashboard" id="character_dashboard" style="display: inline-block;">
			...
			<div class="mini_hero_sheet" style="display: inline-block; width: 300px; padding: 0px;">
				<div style="width: 300px">
					...
					<tbody>
					<tr>
						<td rowspan="4" style="width: 33%; padding: 0px;">
						<img class="hero1_img character_image img img-responsive" src="" /></td>
						<td colspan="2"><span class="hero1_name_short">Name</span></td>
						<td style="width: 20%">⏱️ <span class="timer"></span></td>
					</tr>
					...
					<tr>
						<td colspan="3">
							<!-- Mobile Dice Sheet -->
							<div class="mini_dice_placeholder" style="margin: 2px 0 2px 0;" onclick="roll_dice()">
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body" style="vertical-align: top; margin: 0 auto; text-align: center;">
		...
		<div class="encounter_battle_sheet">
			<table class="table-bordered table-condensed" style="margin: 0 auto; width: 300px;">
				...
				<tr>
					<td style="width: 45%;"><span class="hero1_name_short"></span></td>
					<td style="width: 20%;">❤️ <span class="hero1_health"></span></td>
				</tr>
				<tr>
					<td style="width: 45%;"><span class="hero1_arrows_label">Arrows</span></td>
					<td style="width: 20%;">🏹 <span class="hero1_arrows"></span></td>
				</tr>
				<tr>
					<td colspan="3">
		...
		<!-- Console Sheet -->
		<div class="encounter_console_sheet table-bordered" style="width: 300px; height: 100px; margin: auto; overflow-y:scroll;">
      ...
      <div class="modal-footer">
		...
		<button type="button" id="encounter_shoot_first_arrow_button" class="encounter_shoot_first_arrow_button btn btn-primary" value="Shoot 1st" onclick="battle_action('shoot_first_arrow')">Shoot</button>
		<button type="button" id="encounter_shoot_second_arrow_button" class="encounter_shoot_second_arrow_button btn btn-primary" value="Shoot 2nd" onclick="battle_action('shoot_second_arrow')">Shoot</button>
		<button type="button" id="encounter_ambushed_button" class="encounter_ambushed_button btn btn-primary" value="Ambushed" onclick="battle_action('ambushed')">Ambushed</button>
		<button type="button" id="encounter_combat_button" class="encounter_combat_button btn btn-primary" value="Combat" onclick="battle_action('combat')">Combat</button>
		<button type="button" id="encounter_wait_button" class="encounter_wait_button btn btn-primary" value="Wait" onclick="battle_action('wait')">Wait</button>
		<button type="button" id="encounter_escape_button" class="encounter_escape_button btn btn-primary" value="Escape" onclick="battle_action('escape')">Escape</button>
		<button type="button" id="encounter_fight_button" class="encounter_fight_button btn btn-primary" value="Fight" onclick="roll_encounter_dice('fight')">Attack</button>
      ...
      <div class="modal-body" style="vertical-align: top; margin: 0 auto;">
		<div>
			...
			<table class="table table-condensed">
				<tr>
					<td style="text-align: justify;">Your task is to make it to the Treasure Chamber and escape with the treasures before the timer runs out.</td>
				</tr>
			</table>
			<div style="margin: 10px 0px 10px 0; text-align: left; font-size: 16px; font-weight: bold;">Movement</div>
			<table class="table table-condensed">
				<tr><td style="width: 30%">Desktop</td><td>Use Arrow Keys</td></tr>
				<tr><td>Mobile</td><td>Swipe Game Board</td></tr>
			</table>
			<div style="margin: 10px 0px 10px 0; text-align: left; font-size: 16px; font-weight: bold;">Chamber Description</div>
			<table class="table table-condensed">
				<tr><td style="width: 30%">Normal</td><td>Nothing Special</td></tr>
				<tr><td>Corridor</td><td>Free Movement</td></tr>
				<tr><td>Door</td><td>Normal / Jammed / Trap</td></tr>
				<tr><td>Bridge</td><td>Test AGI to cross</td></tr>
				<tr><td>Portcullis</td><td>Test STR to cross</td></tr>
				<tr><td>SpiderWeb</td><td>Test STR to cross</td></tr>
				<tr><td>Rubbles</td><td>Test AGI to cross</td></tr>
				<tr><td>Pit</td><td>Test LUK to cross</td></tr>
				<tr><td>Rotating</td><td>Rotates 180 Degrees</td></tr>
				<tr><td>Chasm</td><td>[Not Implemented]</td></tr>
				<tr><td>Trap</td><td>Encounter Trap</td></tr>
				<tr><td>Darkness</td><td>Random Movement</td></tr>
				<tr><td>Catacombs</td><td>[Not Implemented]</td></tr>
			</table>
		</div>
      </div>
      <div class="modal-footer">
...
<!-- Rules Modal-->
<div class="modal" id="map_modal" tabindex="-1" role="dialog" aria-labelledby="map_modal_label" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <div style="font-weight: bold; margin: auto; text-align: center; font-size: 20px;">Dungeon Map</div>
      </div>
      <div class="modal-body" style="margin: 0 auto; text-align: center;">
		<div>
			<div style="margin: auto;">
			<div class="bg dungeon_board" style="display: block; width: 800px;"></div>
			</div>
		</div>
      </div>
      <div class="modal-footer">
		<button type="submit" id="rules_close_button" class="btn btn-primary" data-dismiss="modal" onclick="print_board();">Print</button>
		<button type="submit" id="rules_close_button" class="btn btn-primary" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
