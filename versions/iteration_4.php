	...
	<script type="text/javascript">
		...
		function move_hero(p_Direction){
			...
			if (hero1 != undefined && timer > 0 && !game_over) {
				...
				switch(p_Direction) {
					case '4':
						if (valid_space(hero1.x - 1, hero1.y) && valid_movement(p_Direction)) { hero1.x--; previous_direction = p_Direction; } else { return; }
						break;
					case '1':
						if (valid_space(hero1.x, hero1.y + 1) && valid_movement(p_Direction)) { hero1.y++; previous_direction = p_Direction; } else { return; }
						break;
					case '2':
						if (valid_space(hero1.x + 1, hero1.y) && valid_movement(p_Direction)) { hero1.x++; previous_direction = p_Direction; } else { return; }
						break;
		...
		function valid_movement(p_Direction) {
			...
			switch (String(p_Direction)) {
				case '1':
					if (next_hero_chamber != undefined && next_hero_chamber.bottom != '2' && next_hero_chamber.bottom != '4') {
					...
					break;
				case '2':
					if (next_hero_chamber != undefined && next_hero_chamber.left != '2' && next_hero_chamber.left != '4') {
					...
					break;
				case '3':
					if (next_hero_chamber != undefined && next_hero_chamber.top != '2' && next_hero_chamber.top != '4') {
					...
					break;
				case '4':
					if (next_hero_chamber != undefined && next_hero_chamber.right != '2' && next_hero_chamber.right != '4') {
		...
		function roll_dice() {
			return randomNumber = Math.floor(Math.random() * 12+1);
		}
		...
		function cross_bridge(p_Direction) {
			...
			if (moving_backwards(p_Direction)) {
				...
				if (confirm("Do you want to cross the bridge?")) {
					if (dice_roll <= hero1.agility) {
						write_to_console('['+dice_roll+'] You crossed the bridge');
						return true;
					} else {
						write_to_console('['+dice_roll+'] You fell off the bridge');
						update_timer();
						return false;
					}
				} else {
