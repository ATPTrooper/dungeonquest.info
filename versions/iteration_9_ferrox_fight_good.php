...
<head>
	...
	<script type="text/javascript">
		...
		function move_hero(p_direction){
			...
			if (!ferrox_dead) {
				toggle_alert_modal('danger', 'Monster','','The Ferrox is blocking your path');
				return;
			}
		...
		function exit_dungeon() {
			if (check_hero_exit()) {
				...
				setTimeout(function () {
					...
					toggle_alert_modal('success','Exit Dungeon','','You left the dungeon with 0 Gold!!');
				}, 100);
		...
		function roll_ferrox_dice(p_dice) {
			...
			if (total <= hero1.strength) {
				...
				write_to_ferrox_battle_console("You wounded Ferrox 1 Wound");
				write_to_console("You wounded Ferrox 1 Wound");
				...
				$('.dice_total').css('color', '#FFFFFF');
				write_to_ferrox_battle_console("You received 1 wounded from Ferrox");
				write_to_console("You received 1 wounded from Ferrox");
			...
			if (ferrox_dead) {
				$('.ferrox_attack_text').html("You killed the Ferrox!!");
				$('.ferrox_dice_placeholder').attr("onclick", "").unbind("click");
				$('#fight_ferrox_button').attr("onclick", "").unbind("click");
		...
		function fight_ferrox() {
			...
			// different, show ferrox fight modal
			if (!ferrox_dead) {
				$('.ferrox_dice_placeholder').append($('.mini_dice_sheet'));
				
				$('.ferrox_dice_placeholder').attr("onclick", "").unbind("click");
				$('#fight_ferrox_button').attr("onclick", "").unbind("click");
				$(".ferrox_dice_placeholder").on('click', function(){ roll_ferrox_dice(2); });
				$("#fight_ferrox_button").on('click', function(){ roll_ferrox_dice(2); });
			...
			$('.ferrox_attack_text').html("Test your Strength against Ferrox");
			...
			write_to_console("Ferrox has "+ferrox_monster.health+" health");
	...
	<style type="text/css">
		...
		.medium_dungeon_cell { display: inline-block; padding: 0px; margin: 0px; vertical-align: middle; width: 80px; height: 80px; background-size: 100% 100%; }
...
<div class="game" id="game">
	...
	<div class="div-mobile">
		...
		<div class="character_dashboard" id="character_dashboard">
			...
			<!-- Mobile Dice Sheet -->
			<div class="mini_dice_placeholder" onclick="roll_dice(2)">
