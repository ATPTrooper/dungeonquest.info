...
<head>
	...
	<script type="text/javascript" src="/dungeonquest/dungeonquest_alpha.js"></script>
	...
	<script type="text/javascript">
			...
			$('#trap_modal').on('hidden.bs.modal', function () {
				...
				if (debug) {
					if (encounter_queue[0] != undefined) {
						//write_to_console("close modal 1: "+ encounter_queue[0].type);
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								if (encounter_queue.length > 1) {
									//write_to_console("movement[1] started: "+ encounter_queue[0].type);
									encounter_queue.splice(0, 1);
									force_trap_modal();
								} else {
									//write_to_console("movement[0] successful: "+ encounter_queue[0].type);
								...
								//write_to_console("movement not successful: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							if (encounter_queue[0].type == "bridge" || 
								...
								//write_to_console("escape movement: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								return;
					...
					if (encounter_queue[0].type == "trapdoor" ||
						...
						if (encounter_queue[0].resolved == '1') {
							...
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed trap: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								...
								//write_to_console("failed trap: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape trap: "+ encounter_queue[0].type);
							return;
						}
					...
					if (encounter_queue[0].type == "razorwingattack") {
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed chamber encounter: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								...
								//write_to_console("failed chamber encounter: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape chamber encounter: "+ encounter_queue[0].type);
							return;
					...
					if (encounter_queue[0].type == "alreadysearched" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								...
								//write_to_console("failed search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape search: "+ encounter_queue[0].type);
							return;
					...
					if (encounter_queue[0].type == "cryptalreadysearched" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed crypt search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape crypt search: "+ encounter_queue[0].type);
							return;
					...
					if (encounter_queue[0].type == "corpsealreadysearched" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed corpse search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								...
								//write_to_console("failed corpse search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape corpse search: "+ encounter_queue[0].type);
							return;
					...
					if (encounter_queue[0].type == "catacombs") {
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								...
								//write_to_console("passed catacombs search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								...
								//write_to_console("failed catacombs search: "+ encounter_queue[0].type);
								encounter_queue = [];
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
							...
							//write_to_console("cannot escape catacombs: "+ encounter_queue[0].type);
							return;
		...
		function load_characters(){
			for (var i = 0; i < CharactersJSON.length; i++){
				...
				$character_tbody.append('<tr><td rowspan="5" class="character_sheet_image">'+
					'<img class="character_image img img-responsive" src="/dungeonquest/images/second_edition_characters/'+CharactersJSON[i].image_url+'" /></td>'+
		...
		function draw_hero_stats(p_hero_num, p_hero) {
			...
			$('.'+p_hero_num+'_img').attr("src", "/dungeonquest/images/second_edition_characters/"+p_hero.image_url);
		}
		...
		function start_game() {
			...
			write_to_console("Arrow Keys or Swipe to Move");
		...
		function draw_hero(){
			...
			if (hero1 != undefined) {
				$('#dungeon_board_cell_'+(hero1.x)+'_'+(hero1.y)).append('<div style="height: 100%; width: 100%; background-size: 30%; background-position: center; background-repeat: no-repeat; background-image: content(\'/dungeonquest/images/second_edition_characters/'+hero1.image_url+'\')"></div>');
				...
				if (enable_mini_board) {
					$('#mini_board_cell_'+eval(0-x_adjustment)+'_'+eval(0-y_adjustment)+'').append('<div style="height: 100%; width: 100%; display: flex; background-size: 30%; background-position: center; background-repeat: no-repeat; background-image: url(\'/dungeonquest/images/second_edition_characters/'+hero1.image_url+'\')"></div>');
				}
				...
				if (enable_medium_board) {
					$('#medium_board_cell_'+eval(0-x_adjustment)+'_'+eval(0-y_adjustment)+'').append('<div style="height: 100%; width: 100%; display: flex; background-size: 30%; background-position: center; background-repeat: no-repeat; background-image: url(\'/dungeonquest/images/second_edition_characters/'+hero1.image_url+'\')"></div>');
				}
			}
		...
		function draw_chamber(p_direction) {
			...
			if (!is_exit_chamber(hero1) && !is_treasure_chamber(hero1)) {
				...
				if (hero_map[eval(hero1.x*100 + hero1.y)] == undefined) { 
					...
					if (debug) {
						//write_to_console('new tile: ' + randomNumber + " " + new_top + " " + new_right + " " + new_bottom + " " + new_left + " " + randomChamber.type);
					}
		...
		function search_crypt(){
			...
			if (parseInt(curr_hero_chamber.crypt) > 0) {
				if (parseInt(curr_hero_chamber.crypt_searched) < 1) {
					...
					//write_to_console("search id: " + randomNumber + " " + randomDraw.type);
					
					switch (randomDraw.type) {
						case "emptycrypt":
							write_to_console('There is nothing in the Crypt');
							add_encounter('emptycrypt', '0');
							force_trap_modal();
							roll_trap_dice();
							break;
						case "findcryptitem":
							write_to_console('You find an Item in the Crypt');
							...
							break;
						case "hiddentrap":
							write_to_console('You triggered a Hidden Trap');
							trigger_trap("");
							break;
						case "spectralstorm":
							write_to_console('You run into an Army of Undead');
							add_encounter('spectralstorm', '0')
							force_trap_modal();
							break;
						case "oldbones":
							write_to_console('You search the corpse');
					...
					write_to_console('The Crypt has already been searched');
					force_trap_modal();
					roll_trap_dice();
				}
			} else {
		...
		function search_corpse(){
			...
			if (parseInt(curr_hero_chamber.corpse) > 0) {
				if (parseInt(curr_hero_chamber.corpse_searched) < 1) {
					...
					switch (randomDraw.type) {
						case "emptycorpse":
							write_to_console('There is nothing on the Corpse');
							add_encounter('emptycorpse', '0');
							force_trap_modal();
							roll_trap_dice();
							break;
						case "findcorpseitem":
							write_to_console('You find an Item on the Corpse');
							...
							break;
						case "scorpion":
							write_to_console('A Scorpion attacks you!');
					...
					write_to_console('The Corpse has already been searched');
					force_trap_modal();
					roll_trap_dice();
				}
			} else {
		...
		function encounter_chamber() {
			...
			if (event_not_resolved()) { 
				write_to_console("encounter_chamber: event_not_resolved");
				return; 
			}
			...
			write_to_console('Encounter Chamber: '+randomDraw.type);
			//write_to_console("Chamber Event id: " + randomNumber + " " + randomDraw.type);
			
			switch (randomDraw.type) {
				case "deadadventurer":
					write_to_console('You see a dead Adventurer');
					curr_hero_chamber.corpse = '1';
					break;
				case "crypt":
					write_to_console('You see a Crypt');
					curr_hero_chamber.crypt = '1';
					break;
				case "emptychamber":
					write_to_console('The Chamber is Empty!');
					break;
				case "ambush":
					write_to_console('A Monster Attacks You!');
					fight_ferrox();
					break;
				case "hiddentrap":
					write_to_console('You sprung a Hidden Trap');
					trigger_trap("");
					break;
				case "passagedown":
					write_to_console('You see a passage to the Catacombs');
				...
				case "collapse":
					...
					break;
				case "torchout":
					write_to_console('Your Torch Goes Out');
					hero1.torch = '0';
					break;
				case "secretdoor":
					write_to_console('You find a Secret Door');
					curr_hero_chamber.secret_door = '1';
					break;
				case "razorwingattack":
					write_to_console('A Razorwing Attacks you');
					add_encounter('razorwingattack', '0')
					force_trap_modal();
					break;
				case "wizardscurse":
					write_to_console('A Wizard Curses the Dungeon');
					// Magic
					break;
				case "findchamberitem":
					write_to_console('You find an Item');
					add_encounter('findchamberitem', '0');
					add_hero_item(randomDraw.id, randomDraw.name, randomDraw.value, '', '');
					write_to_console('You find ' + randomDraw.name);
					force_trap_modal();
					roll_trap_dice();
				...
				if (parseInt(curr_hero_chamber.searched) < 1) {
					...
					switch (randomDraw.type) {
						...
						case "ferrox":
							...
							break;
						case "trap":
							write_to_console('You search and triggered a Trap!!');
							trigger_trap("");
							break;
						case "passagedown":
							write_to_console('You found a passage to the Catacombs!!');
							curr_hero_chamber.catacombs = '1';
					...
					write_to_console('The room has already been searched');
					force_trap_modal();
					roll_trap_dice();
				}
			} else {
		...
		function add_encounter(type, p_direction) {
			...
			switch (type) {
				...
				// CHAMBER ENCOUNTERS
				case "findchamberitem":
					new_encounter = new Encounter('findcorpseitem', 'Find Corpse Item', '#000000', 'white', 'You find an Item on the Corpse', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "razorwingattack":
					new_encounter = new Encounter('razorwingattack', 'Razorwing Attack', '#000000', 'white', 'You were attacked by a Razorwing', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "torchout":
				...
				case "emptycrypt":
					...
					break;
				case "findcryptitem":
					new_encounter = new Encounter('findcryptitem', 'Find Crypt Item', '#000000', 'white', 'You find an Item in the Crypt', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "oldbones":
				...
				case "gas":
					...
					break;
				case "collapse":
					new_encounter = new Encounter('collapse', 'Hidden Darts', '#000000', 'white', 'Hidden Darts fires at you!!', p_direction);
					encounter_queue.push(new_encounter);
					break;
				case "explosion":
		...
		function roll_trap_dice() {
			...
			if (debug) {
				// write_to_console("roll_trap_dice");
			}
			...
			switch(trap_triggered_type) {
				...
				case "collapse":
					...
					write_to_console('The Ceiling Collapsed!');
					write_to_console('Some passages are blocked and others are opened!');
					$('.trap_text').html("The Ceiling Collapsed! <br/> Some passages are blocked and others are opened!");
					$('.trap_text').css("background-color", "#D1001C");
					$('.trap_text').css("color", "white");
		...
		function event_not_resolved() {
			...
			//alert(1);
			if (encounter_queue.length > 0) {
				//write_to_console("event_not_resolved, encounter queue length > 0");
			...
			//alert(2);
			if (trap_triggered) {
				//write_to_console("event_not_resolved, trap_triggered");
				$('#trap_modal').modal('toggle');
	...
	<div class="div-desktop">
		...
		<div class="dungeon_dashboard">
			...
			<div class="movement_dashboard" id="movement_dashboard">
				...
				<input class="btn btn-primary search_button" type="button" name="search_button" value="Search" onclick="search_room();" />
