	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#encounter_modal').on('hidden.bs.modal', function () {
				...
				// debug
				write_to_debugger("Encounter Modal Closed: " + encounter_queue[0].type);
				write_to_debugger("Encounter Queue Length: " + encounter_queue.length);

				for (var i = 0; i < encounter_queue.length; i++) {
					write_to_debugger("Encounter ["+eval(i+1)+"]: " + encounter_queue[i].type + ", Resolved: "+encounter_queue[i].resolved+", Success: "+encounter_queue[i].success);
				}
