...
<head>
	...
	<script type="text/javascript">
		...
		function valid_movement(p_direction, p_curr_hero_chamber, p_next_hero_chamber, p_prev_hero_chamber) {
			...
			if (false && moving_backwards(p_direction)) {
				...
				switch (String(p_direction)) {
					case '1':
						...
						if (p_curr_hero_chamber.top == '3') {
							add_encounter('door', randomDoor.value, p_direction);
						}
		...
		function add_encounter(type, type_value, p_direction) {
			
			var temp_type_value = type_value;
			if (temp_type_value == undefined) { temp_type_value = 0; }
			
			var new_encounter;
			if (type == "door") {
				switch (temp_type_value) {
					case "1":
		...
		function resolve_obstacle(p_direction, p_obstacle, p_next_obstacle) {
			...
			// passage and door
			} else if (p_obstacle == '1' && temp_next_obstacle == '3') {
			...
			} else if (p_obstacle == '1' && temp_next_obstacle == '5') {
			...
			// door and portcullis
			} else if (p_obstacle == '3' && temp_next_obstacle == '5') {
			...
			// bridge and door
			} else if (p_obstacle == '4' && temp_next_obstacle == '3') {
			...
			} else if (p_obstacle == '4' && temp_next_obstacle == '5') {
			...
			// portcullis and door
			} else if (p_obstacle == '5' && temp_next_obstacle == '3') {
			...
			// pit and door
			} else if (p_obstacle == '6' && temp_next_obstacle == '3') {
			...
			// pit and portcullis
			} else if (p_obstacle == '6' && temp_next_obstacle == '5') {
			...
			// rubble and door
			} else if (p_obstacle == '7' && temp_next_obstacle == '3') {
			...
			} else if (p_obstacle == '7' && temp_next_obstacle == '5') {
			...
			// spiderweb and door
			} else if (p_obstacle == '8' && temp_next_obstacle == '3') {
			...
			} else if (p_obstacle == '8' && temp_next_obstacle == '5') {
				
				new_encounter = new Encounter('bridge', 'Spider Web', '#D1001C', 'white', 'Do you want to move through the Spider Web?', p_direction);
				encounter_queue.push(new_encounter);
			...
			// darkness and door
			} else if (p_obstacle == '9' && temp_next_obstacle == '3') {
				//return move_through_darkness(p_direction, 1);// && open_door(p_direction);
			...
			} else if (p_obstacle == '9' && temp_next_obstacle == '5') {
				//return move_through_darkness(p_direction, 1);// && lift_portcullis(p_direction);
			...
			// darkness and ANYTHING
				...
				//return move_through_darkness(p_direction, 1);
					...
					if (next_hero_chamber != undefined) {
						...
						if (next_hero_chamber.bottom == '2') {
							...
							if (next_hero_chamber.bottom == '3') {
								add_encounter('door', randomDoor.value, p_new_direction);
							} else if (next_hero_chamber.bottom == '5') {
								new_encounter = new Encounter('portcullis', 'Portcullis', '#000000', 'white', 'You bump into a Portcullis in the dark?', p_new_direction);
								encounter_queue.push(new_encounter);
							}
						}
					}
			...
			switch(trap_triggered_type) {
				...
				case "bridge":
					...
					if (total <= hero_agility) {
						...
						// update_hero_health(-hero1.health);
						// if (hero1.health <= 0) {
							// write_to_console('['+total+'] You fall off the Bridge and die!!');
							// $('.trap_text').html("You fall off the Bridge and die!!");
							// game_over = true;
						// }
					}
