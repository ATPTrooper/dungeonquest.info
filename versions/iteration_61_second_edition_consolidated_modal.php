...
<head>
	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#trap_modal').on('hidden.bs.modal', function () {
				...
				$('.trap_hero_sheet_placeholder').show();
				$('.trap_battle_sheet_placeholder').hide();
				$('.battle_sheet_placeholder').append($('.battle_sheet'));
				$('.mini_dice_placeholder').append($('.mini_dice_sheet'));
				$('.trap_button').show();
		...
		function draw_hero(){
			...
			if (hero1 != undefined) {
				$('#dungeon_board_cell_'+(hero1.x)+'_'+(hero1.y)).append('<div style="position: absolute; left: 0; right: 0; top: 0; bottom: 0; margin: auto; height: 80%; width: 80%; background-size: 40%; background-position: '+hero_position+'; background-repeat: no-repeat; transform: rotate('+-hero_orientation+'deg); background-image: url(\''+character_image_directory+hero1.image_url+'\')"></div>');
		...
		function force_trap_modal() {
			...
			if (current_encounter != undefined) {
				...
				// Fight Button
				if (current_encounter.show_fight == '1') { $('.fight_button').show(); } else { $('.fight_button').hide(); }
				if (current_encounter.can_fight == '1') { $('.fight_button').attr("disabled", false); } else { $('.fight_button').attr("disabled", true); }
				
				// Shoot Button
				if (current_encounter.show_shoot == '1') { $('.shoot_button').show(); } else { $('.shoot_button').hide(); }
				if (current_encounter.can_shoot == '1') { $('.shoot_button').attr("disabled", false); } else { $('.shoot_button').attr("disabled", true); }
				...
				if (current_encounter.can_flee == '1') { $('.flee_button').attr("disabled", false); } else { $('.flee_button').attr("disabled", true); }
				...
				switch(current_encounter.type) {
					...
					case "undead":
					case "blackknight":
						$('.trap_hero_sheet_placeholder').hide();
						$('.trap_battle_sheet_placeholder').show();
						$('.trap_battle_sheet_placeholder').append($('.battle_sheet'));
			...
			switch(current_encounter.type) {
				...
				case "crossfire":
					...
					if (parseInt(total) <= parseInt(hero1.defense)) {
						...
						$('.trap_text').html("Your armor deflected the Hidden Arrows!");
						$('.dice_total').css("background-color", "#01E501");
						$('.dice_total').css("color", "white");
						$('.trap_text').css("background-color", "#01E501");
					} else {
						...
						$('.trap_text').html("You were hit by the Hidden Arrows! <br/> You suffer "+wounds+" wounds!");
						$('.dice_total').css("background-color", "#D1001C");
						$('.dice_total').css("color", "white");
						update_hero_health(-wounds);
						if (hero_dead()) {
							write_to_console('['+total+'] You were killed by the Hidden Arrows!');
							$('.trap_text').html("You were killed by the Hidden Arrows!");
							game_over = true;
						}
					}
					break;
				...
				case "wizardscurse":
					...
					play_audio("rotating_chamber");
					$('.trap_text').html("Wizard's Curse: You feel the ground shaking");
					...
					if (p_button == 'fight') { 
						...
						switch (total.toString()) {
							...
							case '2':
								...
								$('.trap_text').html("["+total+"] You received 1 wound");
							...
							case '5':
								...
								$('.dice_total').css('color', 'white');
								write_to_console("["+total+"] You wounded "+current_encounter.title);
								$('.trap_text').html("["+total+"] You wounded "+current_encounter.title);
								update_monster_health(-1);
								play_audio("monster_wounded");
								break;
							case '6':
						...
						if (hero_dead()) {
							...
							$('.trap_text').css("background-color", "#D1001C");
							$('.trap_text').css("color", "white");
							$('.trap_text').html("You were killed by the "+current_encounter.title+"!");
							game_over = true;
						...
						if (parseInt(monster.health) <= 0) {
							...
							$('.trap_text').css("background-color", "#01E501");
							$('.trap_text').css("color", "white");
							$('.trap_text').html("You killed the "+current_encounter.title+"!");
							$('#fight_button').attr("disabled", true);
							$('.flee_button').attr("disabled", true);
							//update_timer();
							return;
						...
						if (total <= hero1.agility) {
							...
							$('.trap_text').html("You escaped! <br/> You received "+monster.escape_penalty+" wounds!");
							play_audio("hero_wounded");
	...
	<style type="text/css">
		...
		.medium_dungeon_cell { display: inline-block; position: relative; padding: 0px; margin: 0px auto; vertical-align: middle; text-align: center; width: 110px; height: 110px; background-size: 100% 100%; }
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body" style="vertical-align: top; margin: 0 auto; text-align: center;">
		...
		<div class="trap_battle_sheet_placeholder"></div>
		<!-- Trap Text -->
      ...
      <div class="modal-footer">
		...
		<button type="button" id="fight_button" class="fight_button btn btn-primary" value="Fight" onclick="roll_trap_dice('fight')">Fight</button>
		<button type="button" id="shoot_button" class="shoot_button btn btn-primary" value="Shoot" onclick="roll_trap_dice('shoot')">Shoot</button>
...
<div style="display: none;">
	<!-- Battle Sheet -->
	<div class="battle_sheet_placeholder">
	<div class="battle_sheet" style="">
		<table class="table-bordered table-condensed" style="margin: 0 auto; width: 300px;">
			...
			<tr>
				<td colspan="3">
					<div class="battle_dice_placeholder" style="cursor: pointer; margin: 5px 0 5px 0;" onclick="roll_trap_dice('fight')"></div>
				</td>
			</tr>
		</table>
	</div>
