	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			...
			$('#trap_modal').on('hidden.bs.modal', function () {
				...
				// debug
				write_to_debugger("Encounter List:");
				if (encounter_queue[0] != undefined) {
					for (var i = 0; i < encounter_queue.length; i++) {
						write_to_debugger("List "+i+1+": " + encounter_queue[0].type + ", Resolved: "+encounter_queue[0].resolved+", Success: "+encounter_queue[0].success);
					}
				}
		...
		function draw_hero(){
			...
			if (current_chamber.treasure_chamber == '1') {
				...
				if (parseInt(curr_hero_chamber.searched) < 1) {
					...
					switch (randomDraw.type) {
						...
						case "emptyroom":
							...
							break;
						case "finditem":
							write_to_console('Search: You find an Item');
							//add_encounter('finditem', '0');
							add_hero_item(randomDraw.id, randomDraw.name, randomDraw.value, '', '');
							write_to_console('You find ' + randomDraw.name);
							//force_trap_modal();
		...
		function search_dragon_chamber(){
			...
			if (debug || hero_map[hero1.x*100 + hero1.y].type == 'treasure') {
				...
				} while (randomDraw == undefined || (dragon_card_already_used(randomNumber) && used_treasure_cards.length < Object.keys(TreasureJSON).length))
	...
	<div class="div-desktop">
		...
		<div class="dungeon_dashboard">
			...
			<div class="movement_dashboard" id="movement_dashboard">
				<input class="btn btn-primary encounter_chamber_button" type="button" name="encounter_chamber_button" value="Encounter Chamber" onclick="encounter_chamber();" />
