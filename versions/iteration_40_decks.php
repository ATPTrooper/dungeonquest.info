...
<head>
	...
	<script type="text/javascript" src="/dungeonquest/dungeonquest_third_edition_revised.js?v=1"></script>
	...
	<script type="text/javascript">
		...
		function remove_door_porticullis_encounters() {
			for (var i = 0; i < encounter_queue.list; i++) {
				if (encounter_queue[i].type == 'door' || encounter_queue[i].type == 'portcullis') {
					...
					if (encounter_queue[0].type == "opendoor" || 
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Encounter Resolved = 1, Success = 1");
								...
								write_to_debugger("Encounter Resolved = 1, Success = 0");
								write_to_debugger("movement not successful: "+ encounter_queue[0].type);
								//encounter_queue = [];
								encounter_queue.splice(0, 1);
								//remove_door_porticullis_encounters();
								trap_triggered = false;
								trap_triggered_type = "";
								update_timer();
								return;
							...
							write_to_debugger("Encounter Resolved = 0, Success = 0");
		...
		function start_game() {
			...
			chamber_deck = new CardDeck(ChamberJSON);
			chamber_event_deck = new CardDeck(ChamberEventJSON);
			catacombs_deck = new CardDeck(CatacombsJSON);
			monster_deck = new CardDeck(MonsterJSON);
			door_deck = new CardDeck(DoorJSON);
			search_deck = new CardDeck(SearchJSON);
			crypt_deck = new CardDeck(CryptJSON);
			corpse_deck = new CardDeck(CorpseJSON);
			trap_deck = new CardDeck(TrapsJSON);
			dragon_deck = new CardDeck(DragonJSON);
			treasure_deck = new CardDeck(TreasureJSON);
		...
		function draw_chamber(p_direction) {
			...
			if (!is_exit_chamber(hero1) && !is_treasure_chamber(hero1)) {
				...
				if (hero_map[eval(hero1.x*100 + hero1.y)] == undefined) { 
					...
					$('.new_chamber').html(randomChamber.key);
					...
					hero_map[eval(hero1.x*100 + hero1.y)] = new Chamber(randomChamber.key, new_top, new_right, new_bottom, new_left, randomChamber.type, eval((p_direction-1)*90), randomChamber.image_url, randomChamber.description);
					hero_map[eval(hero1.x*100 + hero1.y)].searched = 0;
					current_chamber = hero_map[eval(hero1.x*100 + hero1.y)];
					
					write_to_debugger('Draw Tile: ' + randomChamber.key + ": " + new_top + " " + new_right + " " + new_bottom + " " + new_left + " " + randomChamber.type);
		...
		function search_chamber(){
			...
			if (debug || curr_hero_chamber.searchable == '1') {
				...
				if (debug || parseInt(curr_hero_chamber.searched) < 1) {
					...
					// draw random room
					var randomDraw = search_deck.draw_card();
					write_to_console('You searched the Chamber');
					//write_to_debugger("Search: " + randomDraw.key + ": " + randomDraw.type);
					...
					switch (randomDraw.type) {
						...
						case "emptyroom":
							...
							break;
						case "finditem":
							add_hero_item(randomDraw.key, randomDraw.name, randomDraw.value, '', '');
		...
		function search_crypt(){
			...
			if (debug || parseInt(curr_hero_chamber.crypt) > 0) {
				if (debug || parseInt(curr_hero_chamber.crypt_searched) < 1) {
					...
					var randomDraw = crypt_deck.draw_card();
					curr_hero_chamber.crypt_searched = '1';
					
					write_to_console('You searched the Crypt');
					//write_to_debugger("Search Crypt: " + randomDraw.key + ": " + randomDraw.type);
		...
		function search_corpse(){
			...
			if (debug || parseInt(curr_hero_chamber.corpse) > 0) {
				if (debug || parseInt(curr_hero_chamber.corpse_searched) < 1) {
					...
					var randomDraw = corpse_deck.draw_card();
					curr_hero_chamber.corpse_searched = '1';
					
					write_to_console('You searched the Corpse');
					//write_to_debugger("Search Corpse: " + randomDraw.key + ": " + randomDraw.type);
		...
		function search_dragon_chamber(){
			...
			for (var i = 0; i < dragon_deck._used_card_list.length; i++) {
				write_to_debugger("Past Dragon Events: "+dragon_deck._used_card_list[i]);
			}
			...
			if ((debug || hero_map[hero1.x*100 + hero1.y].type == 'treasure') && 
				...
				randomDraw = dragon_deck.draw_unique_card();
	...
	<style type="text/css">
		...
		.loot_sheet { overflow-y:scroll; height: 100px; display: none;}
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-body" style="vertical-align: top; margin: 0 auto;">
		...
		<div>
			<div style="margin: 10px 0px 10px 0; text-align:left; font-size: 16px; font-weight: bold;">Goal</div>
			<table class="table table-condensed table-bordered">
				<tr>
					<td>Get to the Treasure Chamber and get out before time runs out.</td>
				</tr>
			</table>
			
			<div style="margin: 10px 0px 10px 0; text-align:left; font-size: 16px; font-weight: bold;">Controls</div>
			<table class="table table-condensed table-bordered">
				<tr>
					<td style="width: 20%">Desktop</td>
					<td>Use Arrow Keys for movement</td>
				</tr>
				<tr>
					<td>Mobile</td>
					<td>Swipe game board for movement</td>
				</tr>
			</table>
