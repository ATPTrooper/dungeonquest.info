	...
	<script type="text/javascript">
		...
		function encounter_chamber() {
			...
			var curr_hero_chamber = hero_map[hero1.x*100 + hero1.y];
				...
				write_to_console('Encounter Chamber');
		...
		function search_room(){
			...
			if (curr_hero_chamber.type != 0 && 
				...
				curr_hero_chamber.type != 6 && 
				curr_hero_chamber.type != 7 && 
			...
			switch(trap_triggered_type) {
				...
				case "gas":
					...
					if (total > 3) {
						...
						write_to_console('['+total+'] You lose time [ '+ eval(total - 3) +' ]!');
						update_hero_health(-eval(total - 3));
						for (var i = 0; i < total - 3; i++) {
							update_timer();
						}
						if (hero1.health <= 0) {
							write_to_console('['+total+'] You were killed by the Poisonous Gas!');
							$('.trap_text').html("You were killed by the Poisonous Gas!");
							game_over = true;
						}
					} else {
		...
		function event_not_resolved() {
			...
			if (encounter_queue.length > 0) {
				...
				if ($('#trap_modal').data('bs.modal')?.isShown) {
					$('.mini_dice_placeholder').attr("onclick", "").unbind("click");
					$(".mini_dice_placeholder").on('click', function(){ roll_trap_dice(); });
					$('.trap_hero_sheet_placeholder').append($('.mini_hero_sheet'));
					$('#trap_button').attr("disabled", false);
	...
	<div class="div-desktop">
		...
		<div class="dungeon_dashboard">
			...
			<div class="movement_dashboard" id="movement_dashboard">
				<div style="display: inline-block;"><input class="btn btn-primary" type="button" name="encounter_chamber_button" value="Encounter Chamber" onclick="encounter_chamber();" /></div>
				...
				<div style="display: none;"><input class="btn btn-primary" type="button" name="trap_button" value="Trap" onclick="trigger_trap('');" /></div>
