...
<head>
	...
	<script type="text/javascript">
			...
			$('#trap_modal').on('hidden.bs.modal', function () {
				...
				if (encounter_queue[0] != undefined) {
					...
					if (encounter_queue[0].type == "opendoor" || 
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Movement: "+encounter_queue[0].type+": Resolved = 1, Success = 1");
								...
								write_to_debugger("Movement: "+encounter_queue[0].type+": Resolved = 1, Success = 0");
							...
							write_to_debugger("Movement: "+encounter_queue[0].type+": Resolved = 0, Success = 0");
					...
					if (encounter_queue[0].type == "trapdoor" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Trap: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Trap: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Trap: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "vampirebats" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Chamber: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Chamber: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Chamber: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "cryptalreadysearched" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Crypt: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Crypt: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Crypt: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "corpsealreadysearched" ||
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Corpse: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Corpse: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Corpse: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "catacombs") {
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Catacombs: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Catacombs: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Catacombs: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "dragonbreath") {
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Dragon: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Dragon: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Dragon: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "potion") {
						...
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Potion: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Potion: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue.splice(0, 1);
								return;
							}
						} else {
							write_to_debugger("Potion: "+ encounter_queue[0].type+": Resolved = 0");
							return;
						}
					}
					...
					if (encounter_queue[0].type == "torchout") {
						if (encounter_queue[0].resolved == '1') {
							if (encounter_queue[0].success == '1') {
								write_to_debugger("Torchout: "+ encounter_queue[0].type+": Resolved = 1, Success = 1");
								encounter_queue.splice(0, 1);
								return;
							} else {
								write_to_debugger("Torchout: "+ encounter_queue[0].type+": Resolved = 1, Success = 0");
								encounter_queue[0].resolved = '0';
								encounter_queue[0].success = '0';
								return;
							}
						} else {
							write_to_debugger("Torchout: "+ encounter_queue[0].type+": Resolved = 0, Success = 0");
							return;
						}
					}
		...
		function draw_hero(){
			...
			if (hero1 != undefined) {
				$('#dungeon_board_cell_'+(hero1.x)+'_'+(hero1.y)).append('<div style="height: 100%; width: 100%; background-size: 30%; background-position: '+hero_position+'; background-repeat: no-repeat; transform: rotate('+-hero_orientation+'deg); background-image: content(\''+character_image_directory+hero1.image_url+'\')"></div>');
				...
				if (enable_mini_board) {
					$('#mini_board_cell_'+eval(0-x_adjustment)+'_'+eval(0-y_adjustment)+'').append('<div style="height: 100%; width: 100%; display: flex; background-size: 30%; background-position: '+hero_position+'; background-repeat: no-repeat; transform: rotate('+-hero_orientation+'deg); background-image: url(\''+character_image_directory+hero1.image_url+'\')"></div>');
				}
				...
				if (enable_medium_board) {
					$('#medium_board_cell_'+eval(0-x_adjustment)+'_'+eval(0-y_adjustment)+'').append('<div style="height: 100%; width: 100%; display: flex; background-size: 30%; background-position: '+hero_position+'; background-repeat: no-repeat; transform: rotate('+-hero_orientation+'deg); background-image: url(\''+character_image_directory+hero1.image_url+'\')"></div>');
				}
			}
		...
		function force_trap_modal() {
			...
			if (current_encounter != undefined) {
				...
				if (current_encounter.show_flee == '1') { $('.flee_button').show(); } else { $('.flee_button').hide(); }
				if (current_encounter.can_flee == '1') { $('.flee_button').attr("disabled", false); } else { $().attr("disabled", true); }
				if (escape_closed()) { $('.flee_button').attr("disabled", true); }
				...
				if (current_encounter.type == "trapdoor") {
					if (hero1.fell_in_pit == '1') {
						$('.trap_text').html("Climb out of the Pit <br/> [ Test Agility ]");
					} else if (current_encounter.resolved == '1') {
						$('.trap_text').css("background-color", "#01E501");
						$('.trap_text').html("You climbed out of the pit");
					}
				}
				
				// Cave-In
				if (current_encounter.type == "collapse") {
					if (current_encounter.num_dice_roll == 2) {
						$('.trap_text').html("The Ceiling Caved-In! <br/> Death: D6 = 6");
					} else {
						$('.trap_text').html("The Ceiling Caved-In! <br/> Damage: D6");
					}
				}
		...
		function lost_in_darkness(p_dice_roll) {
			...
			$('.trap_text').html("You are lost in the Darkness");
			update_timer();
		}
				...
				if (debug || parseInt(curr_hero_chamber.crypt_searched) < 1) {
					...
					switch (randomDraw.type) {
						...
						case "crypttrap":
							...
							break;
						case "skeleton":
							write_to_console('Crypt: Skeleton [Not Implemented]');
		...
		function encounter_chamber() {
			...
			switch (randomDraw.type) {
				...
				case "trapdoor":
					...
					break;
				case "crossfire":
					write_to_console('Encounter: Hidden Darts!');
				...
				case "vampirebats":
					...
					break;
				case "wizardscurse":
					write_to_console('Encounter: Wizard\'s Curse [Not Implemented]');
					// Magic
					break;
				case "finditem":
					play_audio("gold");
		...
		function fight_monster() {
			...
			var randomDraw = monster_deck.draw_unique_card();
		...
		function add_encounter(type, p_direction) {
			...
			switch (type) {
				...
				case "findcryptitem":
					...
					break;
				case "skeleton":
					//new_encounter = new Encounter('skeleton', 'Skeleton', '#D1001C', 'white', 'You see a Skeleton! <br/> [Roll D6] 1-3 Nothing Happens <br/> [Roll D6] 4-6 Fight Death Warrior', p_direction);
					//encounter_queue.push(new_encounter);
				...
				case "crossfire":
					...
					new_encounter = new Encounter('crossfire', 'Cross Fire', '#D1001C', 'white', 'Hidden Darts fires at you! <br/> [ Damage: 2D6 - Armor ]', p_direction);
					encounter_queue.push(new_encounter);
					break;
		...
		function roll_trap_dice(p_button) {
			...
			switch(current_encounter.type) {
				...
				case "trapdoor":
					...
					if (hero1.fell_in_pit == '1') {
						...
						if (have_rope()) {
							...
							hero1.fell_in_pit = '0'
							$('.trap_text').html("Rope: You climb out of the pit");
							update_timer();
							...
							if (parseInt(total) <= parseInt(hero1.agility)) {
								...
								current_encounter.resolved = '0';
								current_encounter.success = '0';
								$('.trap_text').html("You failed to climb out of the pit");
							}
						}
						...
						if (current_encounter.num_dice_roll == 2) {
							...
							if (parseInt(total) <= parseInt(hero1.agility)) {
								...
								current_encounter.resolved = '0';
								current_encounter.success = '0';
								$('.trap_text').html("You fell into a pit! <br/> Damage: D6");
							}
							...
							$('.trap_text').html("You fell into the trapdoor! <br/> You suffer "+total+" wounds");
							update_hero_health(-total);
							if (hero_dead()) {
				...
				case "gas":
					...
					if (total > 3) {
						play_audio("hero_wounded");
						$('.trap_text').html("You suffer "+ eval(total - 3) +" wounds and lose "+ eval(total - 3) +" time!");
				...
				case "collapse":
					...
					if (current_encounter.num_dice_roll == 2) {
						...
						if (parseInt(total) == 6) {
							...
							$('.trap_text').html("You were killed from the Cave-In!");
							$('.dice_total').css("background-color", "#D1001C");
							$('.dice_total').css("color", "white");
							...
							current_encounter.resolved = '0';
							current_encounter.success = '0';
							$('.trap_text').html("You were injured in the Cave-In! <br/> Damage: D6");
						}
						...
						write_to_console('['+total+'] You were wounded by the Cave-In!');
						write_to_console('['+total+'] You suffer '+total+' wounds!');
						$('.dice_total').css("background-color", "#D1001C");
						$('.dice_total').css("color", "white");
						update_hero_health(-total);
						
						if (hero_dead() || parseInt(dice2) == 6) {
							play_audio("hero_dying");
							write_to_console('['+total+'] You were killed from the Cave-In!');
							$('.trap_text').html("You were killed from the Cave-In!");
							$('.dice_total').css("background-color", "#D1001C");
				...
				case "crossfire":
					...
					if (parseInt(total) <= parseInt(hero1.defense)) {
						write_to_console('['+total+'] Your armor deflected the Hidden Darts!');
						$('.trap_text').html("Your armor deflected the Hidden Darts!");
						$('.dice_total').css("background-color", "#01E501");
						$('.dice_total').css("color", "white");
						$('.trap_text').css("background-color", "#01E501");
					} else {
				...
				case "bridge":
					...
					if (total <= hero_agility) {
						...
						if (hero_dead()) {
							...
							$('.trap_text').html("You are killed by the Fall!");
							game_over = true;
						}
					}
					break;
				...
				case "crypttrap":
					...
					if (total - 3 <= 0) {
						...
						$('.trap_text').html("You avoided the Trap!");
					} else {
						play_audio("hero_wounded");
						current_encounter.resolved = '1';
						current_encounter.success = '0';
						...
						$('.dice_total').css("background-color", "#D1001C");
						$('.dice_total').css("color", "white");
						$('.trap_text').html("You suffer "+eval(total - 3)+" wounds!");
						update_hero_health(-eval(total - 3));
						if (hero_dead()) {
							write_to_console('['+total+'] You are killed by the Trap!');
							$('.trap_text').html("You are killed by the Trap!");
							game_over = true;
						}
					}
				...
				case "skeleton":
					...
					if (total < 4) {
						...
						$('.trap_text').html("Nothing Happens!");
					} else {
						play_audio("monster_wounded");
						current_encounter.resolved = '1';
						current_encounter.success = '0';
						write_to_console('['+total+'] Fight Death Warrior!');
						$('.dice_total').css("background-color", "#D1001C");
						$('.dice_total').css("color", "white");
						$('.trap_text').html("Fight Death Warrior!");
				...
				case "vampirebats":
					...
					if (wounds <= 0) {
						...
						$('.trap_text').html("You evaded the Vampire Bats!");
						$('.dice_total').css("background-color", "#01E501");
						$('.dice_total').css("color", "white");
					} else {
						play_audio("hero_wounded");
						...
						if (hero_dead()) {
							...
							$('.trap_text').html("You are killed by Vampire Bats!");
							game_over = true;
						}
					}
				...
				case "giantspider":
					...
					if (p_button == "flee") {
						...
						if (total < 3) {
							...
							$('.trap_text').html("You flee from the Giant Spider!");
							...
							$('.trap_text').html("You failed to flee from the Giant Spider!");
						}
						...
						if (total <= 3) {
							...
							$('.trap_text').html("You killed the Giant Spider!");
							$('.dice_total').css("background-color", "#01E501");
							$('.dice_total').css("color", "white");
							update_timer();
						} else {
							...
							if (hero_dead()) {
								...
								$('.trap_text').html("You are killed by the Giant Spider!");
								game_over = true;
							}
							update_timer();
						}
					...
					if (current_encounter.show_flee == '1') { $('.flee_button').show(); }
					if (current_encounter.can_flee == '1') { $('.flee_button').attr("disabled", false); } else { $().attr("disabled", true); }
...
<div class="splash_screen" style="display: none;">
	<div style="margin: auto; padding: 20px; cursor: pointer;" onclick="splash_click();">
		<img class="img img-responsive" src="/dungeonquest/images/second_edition/dungeonquest_splash.jpg" />
	</div>
</div>
...
<div class="introduction_screen" style="display: none;">
	<div style="padding: 20px;">
		...
		<div style="margin: auto; cursor: pointer; width: 100%; padding: 0px; margin: 0px;" onclick="introduction_click();">
			<img class="img img-responsive" style="margin: auto;" src="/dungeonquest/images/second_edition/dungeonquest_dragon_2.jpg"/>
		</div>
...
<div class="board_game_screen" id="game" style="width: 100%;">
	<!-- Desktop View -->
	<div class="div-desktop">
		...
		<div class="character_dashboard" id="character_dashboard" style="margin: 10px;">
			...
			<div class="console_sheets">
				...
				<div class="loot_sheet table-bordered">
					<div class="loot_console ">aa</div>
				</div>
				<h5>Console</h5>
				<div class="console_sheet table-bordered">
					<div class="console">aa</div>
				</div>
			</div>
		...
		<div class="dungeon_dashboard">
			...
			<div class="movement_dashboard" id="movement_dashboard">
				<input class="btn btn-primary encounter_button" type="button" name="encounter_button" value="Encounter" onclick="encounter_chamber();" />
				<input class="btn btn-primary door_button" type="button" name="door_button" value="Door" onclick="open_door();" />
				<input class="btn btn-primary trap_button" type="button" name="trap_button" value="Trap" onclick="trigger_trap();" />
...
<div class="modal" id="battle_modal" tabindex="-1" role="dialog" aria-labelledby="battle_modal_label" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-body" style="vertical-align: top; margin: 0 auto;">
			<div><img id="battle_modal_image" class="battle_monster_img img img-responsive" style="width: 300px; margin: 0 auto; display: none;" src="" /></div>
			
			<!-- Battle Sheet -->
			<div class="battle_battle_sheet" style="">
				<table class="table-bordered table-condensed" style="margin: 0 auto; width: 300px;">
					<tr>
						<td rowspan="3"><img class="battle_monster_img img img-responsive" style="margin: 0 auto;" src="" /></td>
						<td style="width: 30%;"><span class="battle_monster_name"></span></td>
						<td style="width: 20%;">❤️ <span class="battle_monster_health"></span></td>
					</tr>
					<tr>
						<td style="width: 30%;"><span class="hero1_name"></span></td>
						<td style="width: 20%;">❤️ <span class="hero1_health"></span></td>
					</tr>
					<tr>
						<td style="width: 30%;">Escape Penalty</td>
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
		...
		<button type="button" id="flee_button" class="flee_button btn btn-primary" value="Flee" onclick="roll_trap_dice('flee')">Flee</button>
...
<div class="modal" id="rules_modal" tabindex="-1" role="dialog" aria-labelledby="rules_modal_label" aria-hidden="true">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      ...
      <div class="modal-footer">
		<button type="submit" id="rules_modal_close_button" class="btn btn-primary" data-dismiss="modal">Close</button>
      </div>
    </div>
  </div>
</div>
